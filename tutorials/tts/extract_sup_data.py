...
def preprocess_ds_for_fastpitch_align(dataloader):
    ...
    print(f"Pitch mean:{pitch_tensor.mean().item()}, Std: {pitch_tensor.std().item()}, Min:{pitch_tensor.min().item()}, Max:{pitch_tensor.max().item()}")
...
def main(cfg):
    ...
    dataloader = torch.utils.data.DataLoader(
        dataset=dataset, batch_size=1, collate_fn=dataset._collate_fn, num_workers=5
    )
